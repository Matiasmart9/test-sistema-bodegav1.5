        ...
        <form onSubmit={handleSubmit} className="p-8 space-y-6">
            ...
            {formData.role === 'cashier' && (
                <div className="pt-4 border-t border-gray-100 animate-fadeIn space-y-4">
                    ...
                    {/* SWITCH VISUAL PARA FIADO */}
                    <div
                        className={`flex items-center justify-between p-4 border rounded-xl cursor-pointer transition-all ${formData.canManageFiado ? 'border-green-200 bg-green-50' : 'border-gray-200 bg-gray-50 hover:border-gray-300'}`}
                        onClick={() => setFormData({...formData, canManageFiado: !formData.canManageFiado})}
                    >
                        <div className="flex items-center gap-3">
                            <div className={`p-2 rounded-lg ${formData.canManageFiado ? 'bg-white text-green-600 shadow-sm' : 'bg-gray-200 text-gray-500'}`}>
                                <HandCoins size={20} />
                            </div>
                            <div>
                                <p className={`font-bold text-sm ${formData.canManageFiado ? 'text-green-800' : 'text-gray-700'}`}>Habilitar Fiado</p>
                                <p className="text-xs text-gray-500">Permite cargar ventas a crédito y cobrar pagos de clientes fiados.</p>
                            </div>
                        </div>
